...
public class WalkerRoom2x2 : AbstractWalkerRoom
{
    public WalkerRoom2x2()
    {
        InitializeRoom();
        GenerateFloor();
        GenerateWalls();
    ...
    protected void Cycle2x2()
    {
        for (int i = 0; i < walkerList.Count; i++)
        {
            ...
            if (walker.Position.y < 31 && walker.Position.y > 0 && walker.Position.x < 31 && walker.Position.x > 0)
            {
                if (roomGrid[(int)walker.Position.x, (int)walker.Position.y] != Grid.FLOOR)
                {
                    roomGrid[(int)walker.Position.x, (int)walker.Position.y] = Grid.FLOOR;
                    floorCount++;
                }
            }
